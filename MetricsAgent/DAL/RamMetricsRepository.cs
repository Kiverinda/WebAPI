...
{ 
    ...
    public class RamMetricsRepository : IRamMetricsRepository
    {
        ...
        {
            using var cmd = new SQLiteCommand(_connection);
            cmd.CommandText = $"DELETE FROM rammetrics WHERE {id}";
            cmd.Prepare();
            cmd.ExecuteNonQuery();
        }
        ...
        {
            using var cmd = new SQLiteCommand(_connection);
            cmd.CommandText = $"UPDATE rammetrics SET available = {item.Available} WHERE {item.Id}";
            cmd.Prepare();
            cmd.ExecuteNonQuery();
        }
        ...
        {
            using var cmd = new SQLiteCommand(_connection);
            cmd.CommandText = $"SELECT * FROM rammetrics WHERE {id}";
            using (SQLiteDataReader reader = cmd.ExecuteReader())
            {
                if (reader.Read())
                {
